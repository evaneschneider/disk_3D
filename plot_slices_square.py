...
dnameout='./m82_eq/512_cool/slices/'
...
for i in range(istart,iend+1):
  ...
  image = a0.imshow(nxy.T, origin='lower', extent=(0, 400, 0, 400), cmap='viridis', vmin=-4.0, vmax=2.0)
  a0.autoscale(False)
  a0.text(100, 350, str(int(t/1000))+' Myr', color='white', horizontalalignment='right')
  a0.hlines(355, 280, 320, color='white')
  a0.text(325, 350, '1 kpc', color='white')
  #colorbar
  ...
  image = a1.imshow(nxz.T, origin='lower', extent=(0, 400, 0, 400), cmap='viridis', vmin=-4.0, vmax=2.0)
  a1.autoscale(False)
  cbaxes = fig.add_axes([xlpad+xlength+xrpad, ybpad, cbwidth, ylength])  
  cbaxes.tick_params(axis='y', which='both', length=4, direction='in')  
  cb = plt.colorbar(image, cax = cbaxes, ticks=[np.arange(-4.0, 2.1, 0.5)])
  cb.set_label("n [$n_h$ $cm^{-3}$]")
  plt.savefig(dnameout+'dslice_'+str(i)+'.png', dpi=300)
  plt.close(fig)
